...
class WhatsAppBehaviorMixin:
    ...
    def _normalize_whatsapp_id(value: Optional[str]) -> str:
        ...
        if ":" in normalized and "@" in normalized:
            # Baileys multi-device JIDs use ``user:device@server`` while
            # mentions and quoted participants use ``user@server``.  The
            # device suffix is not part of the principal identity.
    ...
    # the next trigger. Durable observation below complements this immediate
    ...
    def _whatsapp_history_backfill_enabled(self) -> bool:
        ...
        if configured is not None:
            ...
            return bool(configured)
        return (
            _get_wsecret("WHATSAPP_HISTORY_BACKFILL", default="true") or "true"
        ).lower() in {"true", "1", "yes"}
