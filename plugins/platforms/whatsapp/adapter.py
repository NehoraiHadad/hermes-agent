...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    - history_backfill: Attach a bounded recent ambient-message window to the
      next triggered turn (default: true when require_mention is enabled)
    ...
    def _observe_unmentioned_group_message(
        ...
        try:
            ...
            entry = {
                ...
                # SessionDB stores Unix seconds. Passing an ISO string is
                # treated as malformed and silently falls back to "now",
                # losing the original WhatsApp send time.
        ...
        *,
        ...
        try:
            if not _bypass_gating and not self._should_process_message(data):
                # Keep the existing short-window context behavior, then write
                # an individual durable row when explicit observation policy
                # authorizes this group. The method still returns None, so the
                # message can never reach handle_message/model dispatch.
